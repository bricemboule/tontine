          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Select
                ...
                value={form.method}
                onChange={(e) => ff("method", e.target.value)}
              >
            ...
            <InputField
              ...
              value={form.desc}
              onChange={(e) => ff("desc", e.target.value)}
              placeholder="ex : Cotisation avril 2026, remboursement prêt…"
              error={errors.desc}
            />

            <div
              className={`rounded-md border px-3.5 py-3 text-[13px] text-ink ${isMobile ? "border-info-border bg-info-soft" : "border-warning-border bg-warning-soft"}`}
            >
              {isMobile
            ...
            <div className="flex gap-2">
              ...
              <Button type="submit" loading={loading} fullWidth>
                {loading ? "Initialisation…" : "Initier le paiement"}
              </Button>
            </div>
          </form>
        </Card>
        ...
        <Card className="lg:sticky lg:top-[76px]">
          ...
          <dl className="flex flex-col p-5">
            {recap.map(([l, v], i) => (
              <div
                key={l}
                className={`flex justify-between gap-3 py-[7px] text-[13px] ${i < recap.length - 1 ? "border-b border-line-soft" : ""}`}
              >
