      ...
      <Card className="max-w-2xl">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-5">
          <InputField
            ...
            placeholder="ex : Réunion mensuelle — Mai 2026"
            error={errors.title}
          />
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <InputField
              ...
              type="time"
              value={form.time}
              onChange={(e) => ff("time", e.target.value)}
            />
          </div>
          <InputField
            label="Lieu"
            ...
            placeholder="Adresse, domicile du membre, salle…"
            error={errors.location}
          />
          <Select
            label="Bénéficiaire du tour (optionnel)"
            value={form.beneficiary}
            onChange={(e) => ff("beneficiary", e.target.value)}
          >
          ...
          {form.date && (
            <div className="rounded-md border border-info-border bg-info-soft px-3.5 py-3 text-[13px] text-ink">
              SMS + WhatsApp envoyés automatiquement à tous les membres actifs (
