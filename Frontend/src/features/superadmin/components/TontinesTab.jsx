...
export default function TontinesTab({
  api,
  toast,
  onTontinesChange,
  onOpenAdmins,
  ...
  const loadData = async () => {
    const [tontineRows, adminRows] = await Promise.all([
      api.getTontines(),
  ...
  const validate = () => {
    ...
    if (!form.admin_user_id)
      next.admin_user_id = "Sélectionnez un administrateur";
    return next;
  };
  ...
  const doCreate = async () => {
    ...
    try {
      const payload = {
        name: form.name.trim(),
        admin_user_id: parseInt(form.admin_user_id, 10),
      };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (t) => <span className="text-ink-muted">{getAdminLabel(t)}</span>,
    },
    {
      key: "schema",
      header: "Schéma",
      render: (t) => (
        <span className="font-display text-[12.5px] font-bold text-primary-600">
          {getSchemaLabel(t)}
        </span>
      ),
    },
    {
      ...
      align: "right",
      render: (t) => (
        <span className="font-display font-bold">{getMembersCount(t)}</span>
      ),
    },
    {
      key: "status",
      ...
      {showForm ? (
          ...
          <CardBody className="flex flex-col gap-4">
            ...
            {form.name.trim() && (
              <div className="rounded-md bg-surface-2 px-3.5 py-2.5">
                ...
                <div className="mt-0.5 font-display text-[13px] font-bold text-primary-600">
                  tontine_{slugify(form.name)}
                </div>
              </div>
            )}
            ...
            {admins.length === 0 ? (
              <div className="rounded-md border border-line bg-surface-2 p-4">
                ...
                <div className="mt-1 text-[11.5px] text-ink-subtle">
                  Chaque administrateur ne peut gérer qu'une seule tontine.
                  Libérez-en un ou créez un nouveau compte admin.
                </div>
                <Button className="mt-3" size="sm" onClick={onOpenAdmins}>
              ...
              <Select
                ...
                onChange={(e) => ff("admin_user_id", e.target.value)}
        ...
        <DataTable
          ...
          empty={{
            ...
            message: "Aucune tontine créée pour le moment.",
          }}
        />
      )}
    </>
