...
const TYPES = [
  {
    ...
    fine: 2000,
    desc: "Cotisation payée après la date limite",
  },
  {
    type: "Absence réunion",
    fine: 5000,
    desc: "Absence non justifiée à une réunion",
  },
  {
    type: "Comportement",
    fine: 10000,
    desc: "Comportement contraire au règlement",
  },
  {
    type: "Non-respect règlement",
    fine: 15000,
    desc: "Infraction grave au règlement intérieur",
  },
  {
    type: "Suspension",
    fine: 0,
    desc: "Exclusion temporaire de toutes activités",
  },
];
...
export default function SanctionCreate() {
  ...
  useEffect(() => {
    api
      .getMembers()
      .then((m) =>
        setMembers(m.filter((x) => ["active", "suspended"].includes(x.status))),
      )
      .catch(() => {});
  }, [api]);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const s = await api.proposeSanction({
        ...
        <Card>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-5">
            ...
            >
              ...
              {members.map((m) => (
                <option key={m.id} value={m.id}>
                  {m.name} ({m.status === "suspended" ? "suspendu" : "actif"})
                </option>
              ))}
            </Select>
                ...
                >
                  ...
                  <span
                    className={cn(
                      ...
                      t.fine > 0 ? "text-danger" : "text-ink-subtle",
                    )}
                  >
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <InputField
                label="Amende personnalisée (FCFA)"
                type="number"
                min="0"
                value={form.fine}
                onChange={(e) => ff("fine", parseFloat(e.target.value) || 0)}
              />
              <InputField
                label="Date d'effet"
                type="date"
                value={form.date}
                onChange={(e) => ff("date", e.target.value)}
              />
            </div>

            <div className="rounded-md border border-warning-border bg-warning-soft px-3.5 py-3 text-[13px] text-ink">
              Le Président sera notifié immédiatement. La sanction ne sera
              appliquée qu'après sa validation.
            </div>
        ...
        {selectedMember && (
          <Card className="lg:sticky lg:top-[76px]">
            ...
            <div className="p-5 text-center">
              ...
              <div className="mt-2.5 font-display text-[15px] font-bold text-ink">
                {selectedMember.name}
              </div>
