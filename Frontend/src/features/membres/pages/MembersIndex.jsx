...
export default function MembersIndex({ caps = {} }) {
  ...
  // Validation en un clic depuis la liste (pas de motif requis, contrairement
  // au rejet/suspension qui passent par une page de confirmation).
  const doValidate = async (id) => {
    setBusyId(id);
    try {
      await api.validateMember(id, "approve");
      setMembers((p) => p.map((x) => (x.id === id ? { ...x, status: "active" } : x)));
      toast("Membre validé ✓");
    ...
    ...(caps.moderate ? [{
      ...
      render: (m) => (
        <div className="flex justify-end gap-1.5" onClick={(e) => e.stopPropagation()}>
          {m.status === "pending" && (
            <>
              <Button variant="success" size="xs" iconLeft={<Check size={14} />} loading={busyId === m.id} onClick={() => doValidate(m.id)}>Valider</Button>
