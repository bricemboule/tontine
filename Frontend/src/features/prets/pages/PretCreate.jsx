...
export default function PretCreate() {
  ...
  const sim =
    form.amount > 0
      ? calcLoan(parseFloat(form.amount) || 0, form.rate, form.months)
      : null;
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const l = await api.createLoan({
        member_id: parseInt(form.mid),
    ...
    <>
      ...
      <div className="grid items-start gap-4 lg:grid-cols-[1fr_320px]">
        <Card>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-5">
            <Select
              ...
              value={form.mid}
              onChange={(e) => ff("mid", e.target.value)}
              error={errors.mid}
            >
            ...
            <InputField
              ...
              max="100"
              value={form.rate}
              onChange={(e) => ff("rate", parseFloat(e.target.value) || 0)}
            />

            <div className="flex flex-col gap-2">
              <label className="text-[13px] font-semibold text-ink">
                Durée de remboursement :{" "}
                <strong className="text-primary-700">{form.months} mois</strong>
              </label>
              <input
                type="range"
                min="1"
                ...
                step="1"
                value={form.months}
                onChange={(e) => ff("months", parseInt(e.target.value))}
            ...
            <div className="rounded-md border border-info-border bg-info-soft px-3.5 py-3 text-[13px] text-ink">
              Le Président sera notifié pour approuver ou rejeter cette demande.
              Les fonds seront débloqués après approbation.
            </div>
        ...
        <Card className="lg:sticky lg:top-[76px]">
          <CardHeader>
            <CardTitle>Simulation en temps réel</CardTitle>
          </CardHeader>
          <div className="p-5">
            {!sim ? (
              ...
              <>
                <dl className="flex flex-col">
                  {[
                    [
                      "Montant demandé",
                      fmtCFA(parseFloat(form.amount) || 0),
                      false,
                    ],
                    [
                      "Taux",
                      `${form.rate}%/an → ${(form.rate / 12).toFixed(2)}%/mois`,
                      false,
                    ],
                    ...
                    <div
                      key={l}
                      className={`flex justify-between gap-3 py-[7px] text-[13px] ${i < arr.length - 1 ? "border-b border-line-soft" : ""}`}
                    >
                      <dt className="text-ink-muted">{l}</dt>
                      <dd
                        className={`font-display tabular-nums ${bold ? "font-extrabold text-ink" : "font-medium text-ink"}`}
                      >
                        {v}
                      </dd>
                    </div>
                ...
                <div className="mt-4 border-t border-line pt-3">
                  <div className="mb-2 text-[10.5px] font-bold uppercase tracking-[0.08em] text-ink-subtle">
                    Échéancier
                  </div>
                  <div className="max-h-[150px] overflow-y-auto">
                    {Array.from({ length: form.months }, (_, i) => (
                      <div
                        key={i}
                        className="flex justify-between border-b border-line-soft py-1 text-[12px] last:border-0"
                      >
                        <span className="text-ink-subtle">Mois {i + 1}</span>
                        <span className="font-display font-semibold tabular-nums text-ink">
                          {fmtCFA(
                            i === form.months - 1
                              ? sim.total - sim.monthly * (form.months - 1)
                              : sim.monthly,
                          )}
                        </span>
                      </div>
